...
class CNN(tf.keras.Model):
  def __init__(self):
    ...
    # 1st convoultion layer(image -> 64 feature (mapping))
    ...
    # 1st Fully Connected Layer (32x32 image -> 8x8x128 feature map)

    # mapping to 384 feature
    self.flatten_layer = tf.keras.layers.Flatten()
    self.fc_layer_1 = tf.keras.layers.Dense(384, activation='relu')
    self.dropout = tf.keras.layers.Dropout(0.2)

    # 2nd Fully Connected Layer (384 feature -> 10 class(mapping))
...
# define function to print model accuracy
@tf.function
